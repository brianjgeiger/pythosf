...
class Session:
    def __init__(self, api_base_url, token=None, auth=None, default_version=None, config=None):
    ...
    def json_api_request(self, url, method=None, item_id=None, item_type=None, attributes=None, raw_body=None,
                         query_parameters=None, fields=None, headers=None, retry=True, token=None, auth=None):
        request_body = {}
        if not token:
            token = self.token
        if not auth:
        ...
        while keep_trying:
            keep_trying = False
            combined_headers = combine_headers(self.base_headers(token=token), headers)
            try:
                if method == 'GET':
                    response = requests.get(url, params=query_parameters, auth=auth, headers=combined_headers)
                elif method == 'POST':
                    response = requests.post(url, params=query_parameters, json=request_data, data=raw_body,
                                             auth=auth, headers=combined_headers)
                elif method == 'PUT':
                    ...
                    response = requests.delete(url, params=query_parameters, auth=auth,
    ...
    def get(self, url, query_parameters=None, headers=None, retry=True, token=None, auth=None, retrieve_all=False):
        response = self.json_api_request(url=url, method="GET", query_parameters=query_parameters,
                                     headers=headers, retry=retry, token=token, auth=auth)
    ...
    def put(self, url, item_id=None, item_type=None, query_parameters=None, attributes=None, headers=None,
        ...
        return self.json_api_request(url=url, method="PUT", item_id=item_id, item_type=item_type,
                                     ...
                                     retry=retry, raw_body=raw_body, token=token, auth=auth)
    ...
    def delete(self, url, item_type, query_parameters=None, attributes=None, headers=None,
               retry=True, token=None, auth=None):
        self.json_api_request(url=url, method="DELETE", item_type=item_type, attributes=attributes,
                              query_parameters=query_parameters, headers=headers, retry=retry, token=token, auth=auth)
        return None
...
class Node(APIDetail):
    ...
    def create(self, title, category="project", description=None, public=None, tags=None,
               template_from=None, query_parameters=None, token=None, auth=None):
        saved_args = locals()
        ...
        response = self.session.post(url='/v2/nodes/', item_type=self.type, attributes=attributes,
                                     query_parameters=query_parameters, token=token, auth=auth)
        if response:
    ...
    def delete(self, query_parameters=None, token=None, auth=None):
        if self.id is None:
            return None
        else:
            self_url = self.links.self
            self.session.delete(url=self_url, item_type=self.type, query_parameters=query_parameters,
                                token=token, auth=auth)
            self.id = None
            return None

    def get(self, query_parameters=None, token=None, auth=None):
        url = None
        if self.self_link:
            url = self.self_link
        elif self.links:
        ...
        if url:
            response = self.session.get(url=url, query_parameters=query_parameters, token=token, auth=auth)
            if response:
                self._update(response=response)
        else:
            raise ValueError("No url or id to get. Set the id or self_link then try to get.")

    def get_providers(self, query_parameters=None, token=None, auth=None):
...
class File(APIDetail):
    def __init__(self, session, node=None, location=None, name=None, data=None, wb_data=None, token=None, auth=None):
        super().__init__(session=session, data=data)
        if wb_data is not None:
            self._update_from_wb(wb_data=wb_data, token=token, auth=auth)
        elif data is None:
            self.name = name
    ...
    def _update_from_wb(self, wb_data, token=None, auth=None):
        ...
        response = self.session.get(url=osf_url, token=token, auth=auth)
        self._update(response=response)

    def get(self, url=None, query_parameters=None, token=None, auth=None):
        if url:
            self.location = url
        elif self.links.self:
            self.location = self.links.self

        response = self.session.get(url=self.location, query_parameters=query_parameters, token=token, auth=auth)
        self._update(response=response)

    def download(self, query_parameters=None, token=None, auth=None):
        url = self.links.download
        return self.session.get(url=url, query_parameters=query_parameters, token=token, auth=auth)

    def upload(self, data, query_parameters=None, token=None, auth=None):
        ...
        return self.session.put(url=url, query_parameters=combined_query_parameters, raw_body=data,
                                token=token, auth=auth)

    def _move_or_copy(self, to_folder, action, rename=None, conflict=None, query_parameters=None, token=None, auth=None):
        body = {
            'action': action,
            'path': to_folder.path,
        ...
        url = self.links.move
        return self.session.post(url=url, raw_body=raw_body, query_parameters=query_parameters, token=token, auth=auth)

    def move(self, to_folder, rename=None, conflict=None, query_parameters=None, token=None, auth=None):
        moved_file = self._move_or_copy(to_folder=to_folder, action='move', rename=rename, conflict=conflict,
                                        query_parameters=query_parameters, token=token, auth=auth)
        self._update_from_wb(wb_data=moved_file, token=token, auth=auth)

    def copy(self, to_folder, rename=None, conflict=None, query_parameters=None, token=None, auth=None):
        new_file = self._move_or_copy(to_folder=to_folder, action='copy', rename=rename, conflict=conflict,
                                      query_parameters=query_parameters, token=token, auth=auth)
        return File(session=self.session, wb_data=new_file, token=token, auth=auth)

    def delete(self, query_parameters=None, token=None, auth=None):
        url = self.links.delete
        return self.session.delete(url=url, item_type=self.type, query_parameters=query_parameters, token=token, auth=auth)

    def rename(self, name, query_parameters=None, token=None, auth=None):
        body = {
            'action': 'rename',
            'rename': name
        }
        raw_body = json.JSONEncoder().encode(body)
        url = self.links.move
        response = self.session.post(url=url, raw_body=raw_body, query_parameters=query_parameters,
                                     token=token, auth=auth)
        self._update(response=response)
...
class Folder(File):
    def __init__(self, session, node=None, location=None, name=None, data=None, wb_data=None, token=None, auth=None):
        super().__init__(session=session, node=node, location=location, name=name, data=data,
                         wb_data=wb_data, token=token, auth=auth)
        self.type = "files"
        self.files = []

    def get(self, token=None, auth=None, append=False, query_parameters=None, retrieve_all=False):
        url = self.relationships.files['links']['related']['href']
        response = self.session.get(url=url, token=token, auth=auth, retrieve_all=retrieve_all,
    ...
    def list(self, token=None, auth=None, append=False, query_parameters=None, retrieve_all=False):
        return self.get(token=token, auth=auth, append=append, query_parameters=query_parameters,
                        retrieve_all=retrieve_all)

    def create(self, name, query_parameters=None, token=None, auth=None):
        ...
        new_folder_data = self.session.put(url=url, query_parameters=combined_query_parameters, raw_body='',
                                           token=token, auth=auth)
        return Folder(session=self.session, wb_data=new_folder_data, token=token, auth=auth)

    def upload(self, name, data, query_parameters=None, token=None, auth=None):
        url = self.links.upload
        query_parameters = query_parameters or {}
        upload_query_parameters = {
            'kind': 'file',
        ...
        combined_query_parameters = {**query_parameters, **upload_query_parameters}
        new_file_data = self.session.put(url=url, query_parameters=combined_query_parameters, raw_data=data,
                                         token=token, auth=auth)
